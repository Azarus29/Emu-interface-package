...
angular.module('emuwebApp')
	.directive('level', function ($timeout, $animate, bufferService, Drawhelperservice, fontScaleService, LevelService, AnnotService,appStateService) {
		return {
			...
			link: function postLink(scope, element) {
				...
				scope.levelDef = ConfigProviderService.getLevelDefinition(scope.level.name); //FIXME : Remove ConfigProviderService
				...
				scope.$watch('anots.getAnnot()', function (newValue, oldValue) {
				...
				scope.drawLevelDetails = function () {
					...
					if (scope.level.name === curAttrDef) {
						if (isOpen) {
							fontScaleService.drawUndistortedTextTwoLines(ctx, scope.level.name, '(' + scope.level.type + ')', fontSize, fontFamily, 4, ctx.canvas.height / 2 - fontSize * scaleY, "#000", true);
						}
						else {
							fontSize -= 2;
							fontScaleService.drawUndistortedText(ctx, scope.level.name, fontSize, fontFamily, 4, ctx.canvas.height / 2 - (fontSize * scaleY / 2), "#000", true);
						}
					} else {
						fontScaleService.drawUndistortedTextTwoLines(ctx, scope.level.name + ':' + curAttrDef, '(' + scope.level.type + ')', fontSize, fontFamily, 4, ctx.canvas.height / 2 - fontSize * scaleY, "#000", true);
					}
